...
require_admin();
$id = (int)($_GET['id'] ?? 0);
$mainStmt = $pdo->prepare('SELECT main_image FROM product_groups WHERE id = :id');
$mainStmt->execute(['id' => $id]);
$main = $mainStmt->fetchColumn();
if ($main) {
    $file = __DIR__ . '/../public_html/uploads/groups/' . $main;
    if (is_file($file)) {
        unlink($file);
    }
}
$imageStmt = $pdo->prepare('SELECT image_path FROM product_group_images WHERE group_id = :id');
